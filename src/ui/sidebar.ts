...
const unitOrder = ['harvester', 'tanker', 'hauler', 'logger', 'repair', 'envoy', 'aegis', 'recon', 'infantry', 'rocket', 'transport', 'sentrypod', 'strike', 'artillery', 'walker', 'harrier', 'aircraft', 'hunter', 'spectre', 'borer', 'cartman', 'kenny', 'stan', 'kyle', 'jedi', 'sith', 'bountyhunter', 'droideka'];
...
function iconCanvas(kind: 'b' | 'u', type: string): HTMLCanvasElement {
  ...
  if (kind === 'b') {
    ...
    else if (type === 'shieldgen') { g.fillStyle = 'rgba(170,220,255,.85)'; g.beginPath(); g.arc(cx, cy, 4, 0, 7); g.fill(); g.strokeStyle = '#9fdcff'; g.lineWidth = 1.4; for (let i = 0; i < 3; i++) { g.beginPath(); g.arc(cx, cy, 6 + i * 3, Math.PI * 1.15, Math.PI * 1.85); g.stroke(); } }
    ...
    else if (type === 'transport') { g.fillRect(11, 6, 18, 14); g.strokeRect(11, 6, 18, 14); g.fillStyle = 'rgba(160,195,220,.6)'; g.fillRect(14, 8, 12, 3); g.strokeStyle = '#9fd9cc'; g.strokeRect(15, 14, 10, 5); }
    else if (type === 'sentrypod') { g.fillRect(12, 9, 16, 11); g.strokeRect(12, 9, 16, 11); g.fillStyle = '#2c3744'; g.beginPath(); g.arc(cx, cy + 2, 4, 0, 7); g.fill(); g.strokeStyle = '#aebcc8'; g.lineWidth = 2.2; g.beginPath(); g.moveTo(cx, cy + 2); g.lineTo(cx + 2, cy - 7); g.stroke(); }
    else if (type === 'cartman') { g.fillStyle = '#c0392b'; g.beginPath(); g.ellipse(cx, cy + 3, 8, 7, 0, 0, 7); g.fill(); g.fillStyle = '#e7c9a0'; g.beginPath(); g.arc(cx, cy - 3, 5, 0, 7); g.fill(); g.fillStyle = '#27a3b8'; g.beginPath(); g.arc(cx, cy - 5, 5.5, Math.PI, 0); g.fill(); g.fillStyle = '#f2d23a'; g.beginPath(); g.arc(cx, cy - 11, 2, 0, 7); g.fill(); }
    else if (type === 'kenny') { g.fillStyle = '#e07a23'; g.beginPath(); g.ellipse(cx, cy + 2, 7, 7, 0, 0, 7); g.fill(); g.fillStyle = '#e88a33'; g.beginPath(); g.arc(cx, cy - 4, 5.5, 0, 7); g.fill(); g.fillStyle = '#1c1c1c'; g.beginPath(); g.arc(cx, cy - 3, 2.6, 0, 7); g.fill(); }
    else if (type === 'stan') { g.fillStyle = '#6f4a2b'; g.beginPath(); g.ellipse(cx, cy + 3, 8, 7, 0, 0, 7); g.fill(); g.fillStyle = '#e7c9a0'; g.beginPath(); g.arc(cx, cy - 3, 5, 0, 7); g.fill(); g.fillStyle = '#2f6fd0'; g.beginPath(); g.arc(cx, cy - 5, 5.5, Math.PI, 0); g.fill(); g.fillStyle = '#d23a2f'; g.beginPath(); g.arc(cx, cy - 11, 2, 0, 7); g.fill(); }
    else if (type === 'kyle') { g.fillStyle = '#e08a2a'; g.beginPath(); g.ellipse(cx, cy + 3, 8, 7, 0, 0, 7); g.fill(); g.fillStyle = '#e7c9a0'; g.beginPath(); g.arc(cx, cy - 3, 5, 0, 7); g.fill(); g.fillStyle = '#2f9e4f'; g.beginPath(); g.arc(cx, cy - 5, 5.5, Math.PI, 0); g.fill(); g.fillStyle = '#2f9e4f'; g.fillRect(cx - 6, cy - 6, 2, 4); g.fillRect(cx + 4, cy - 6, 2, 4); }
    else if (type === 'jedi') { g.fillStyle = '#5a4632'; g.beginPath(); g.moveTo(cx - 6, cy + 7); g.lineTo(cx + 6, cy + 7); g.lineTo(cx + 3, cy - 5); g.lineTo(cx - 3, cy - 5); g.closePath(); g.fill(); g.fillStyle = '#e7c9a0'; g.beginPath(); g.arc(cx, cy - 6, 3.5, 0, 7); g.fill(); g.strokeStyle = '#7fe0ff'; g.lineWidth = 2; g.beginPath(); g.moveTo(cx + 7, cy + 6); g.lineTo(cx + 11, cy - 7); g.stroke(); g.strokeStyle = 'rgba(190,235,255,.7)'; g.lineWidth = 4; g.beginPath(); g.moveTo(cx + 7, cy + 6); g.lineTo(cx + 11, cy - 7); g.stroke(); }
    else if (type === 'sith') { g.fillStyle = '#1a1320'; g.beginPath(); g.moveTo(cx - 6, cy + 7); g.lineTo(cx + 6, cy + 7); g.lineTo(cx + 3, cy - 6); g.lineTo(cx - 3, cy - 6); g.closePath(); g.fill(); g.fillStyle = '#0a0a0a'; g.beginPath(); g.arc(cx, cy - 6, 4, Math.PI * 1.05, Math.PI * 1.95); g.fill(); g.strokeStyle = '#c79bff'; g.lineWidth = 1.5; g.beginPath(); g.moveTo(cx + 7, cy - 7); g.lineTo(cx + 3, cy - 2); g.lineTo(cx + 7, cy - 1); g.lineTo(cx + 2, cy + 5); g.stroke(); }
    else if (type === 'bountyhunter') { g.fillStyle = '#5a6470'; g.beginPath(); g.ellipse(cx, cy + 1, 6, 7, 0, 0, 7); g.fill(); g.fillStyle = '#3a4450'; g.beginPath(); g.arc(cx, cy - 4, 4, 0, 7); g.fill(); g.fillStyle = '#2aa9c0'; g.fillRect(cx - 2.5, cy - 5, 5, 1.6); g.fillStyle = '#39424f'; g.fillRect(cx - 8, cy - 4, 3, 8); g.fillRect(cx + 5, cy - 4, 3, 8); g.fillStyle = '#ffb347'; g.beginPath(); g.arc(cx - 6.5, cy + 5, 1.5, 0, 7); g.fill(); g.beginPath(); g.arc(cx + 6.5, cy + 5, 1.5, 0, 7); g.fill(); }
    else if (type === 'droideka') { g.strokeStyle = 'rgba(127,208,255,.8)'; g.lineWidth = 1.4; g.beginPath(); g.arc(cx, cy, 9, 0, 7); g.stroke(); g.fillStyle = '#7a6a4a'; g.beginPath(); g.arc(cx, cy + 1, 3.5, 0, 7); g.fill(); g.strokeStyle = '#b9a778'; g.lineWidth = 2; for (const a of [-0.6, 0.6, Math.PI]) { g.beginPath(); g.moveTo(cx, cy + 1); g.lineTo(cx + Math.cos(a) * 7, cy + 1 + Math.sin(a) * 7); g.stroke(); } g.fillStyle = '#e8483a'; g.beginPath(); g.arc(cx + 5, cy - 4, 1.4, 0, 7); g.fill(); g.beginPath(); g.arc(cx - 5, cy - 4, 1.4, 0, 7); g.fill(); }
