...
export class BattleScene extends Phaser.Scene {
  ...
  private setupKeys() {
    ...
    kb.on('keydown', (e: KeyboardEvent) => {
      ...
      else if (k === 'u') ejectGarrison();        // unload a garrisoned building or a loaded transport
      else if (k === 'y') toggleDeploy();         // deploy a Sentry Pod into a turret (or pack one up)
  ...
  private syncUnit(u: Unit) {
    ...
    const vis = canSee(u) && !cloakedToPlayer(u);                    // a cloaked enemy Spectre is invisible to you
    const wv = this.cameras.main.worldView;                          // viewport culling — skip off-screen sprites
